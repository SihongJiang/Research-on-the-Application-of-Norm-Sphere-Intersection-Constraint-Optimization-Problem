# -*- coding: utf-8 -*-
"""
Created on Wed May 18 10:26:30 2022
...
u1 = [0 for i in range(50)];  u2 = [0 for i in range(50)]; u3 = [0 for i in range(50)]
for i in range(50):
...
w1 = [0 for i in range(200)]; w2 = [0 for i in range(500)]; w3 = [0 for i in range(700)]
for i in range(75):
...
#初始化E_j
E1 = [[0 for i in range(50)] for j in range(200)]
E2 = [[0 for i in range(50)] for j in range(500)]
E3 = [[0 for i in range(50)] for j in range(700)]
...
X1 = [[0 for i in range(200)] for j in range(50)]; X2 = [[0 for i in range(500)] for j in range(50)]; X3 = [[0 for i in range(700)] for j in range(50)];
for i in range(50):
    for j in range(200):
        if j<75:
...
def eplb(v,n,z,lambda0):
    ...
    if (v[0] !=0):
        rho_1=1;
        s_1=abs(v[0]);x[V_i]=abs(v[0]);v_max=abs(v[0])
        V_i = V_i+1
    else:
        rho_1=0;
        s_1=0;v_max=0;   
    for i in range(1,n):
        if (v[i]!=0):
            x[V_i]=abs(v[i]); s_1=s_1+x[V_i]; rho_1 = rho_1+1
            if (x[V_i] > v_max):
                v_max=x[V_i]
            V_i = V_i+1
    ...
    Lambda=lambda0; 
    if ( (Lambda<lambda_2) and (Lambda> lambda_1) ):
        ...
        while (i <= j):          
            ...
            if (i<j):
                ...
                i = i+1;  j = j-1;
        rho=V_i_e-j;  rho = rho+rho_2;  s = s+s_2
    ...
    while (flag==0):
        ...
        if (abs(lambda_T-lambda_S) <= delta):
            Lambda=lambda_T; flag=1
            break
        #set lambda as the middle point of lambda_T and lambda_S
        Lambda=(lambda_T+lambda_S)/2
        s_T=0;s_S=0;s=0
        rho_T=0;rho_S=0;rho=0
        i=V_i_b; j=V_i_e;
        while (i <= j):         
            while( (i <= V_i_e) and (x[i] <= Lambda) ):
                if (x[i]> lambda_T):
                    s_T = s_T+x[i]; rho_T = rho_T+1
                i = i+1
            while( (j>=V_i_b) and (x[j] > Lambda) ):
                if (x[j] > lambda_S):
                    s_S=s_S+x[j]; rho_S = rho_S+1
                else:
                    s=s+x[j]; rho=rho+1
                j=j-1
            if (i<j):
                if (x[i] > lambda_S):
                    s_S=s_S+x[i]; rho_S=rho_S+1
                else:
                    s=s+x[i]; rho=rho+1
                if (x[j]> lambda_T):
                    s_T=s_T+x[j]; rho_T=rho_T+1
                temp=x[i]; x[i]=x[j];  x[j]=temp
                i=i+1; j=j-1
        
        s_S=s_S+s_2; rho_S=rho_S+rho_2
        s=s+s_S; rho=rho+rho_S
        s_T=s_T+s; rho_T=rho_T+rho
        ...
        if ( abs(f_lambda)< delta ):
            flag=1
            break
        if ( abs(f_lambda_S)< delta ):
            Lambda=lambda_S; flag=1
            break
        if ( abs(f_lambda_T)< delta ):
            Lambda=lambda_T; flag=1;
            break     
        if (f_lambda <0):
            ...
            V_i_e=j;  i=V_i_b
            while (i <= j):
                while( (i <= V_i_e) and (x[i] <= lambda_T) ):
                    i=i+1
                while( (j>=V_i_b) and (x[j] > lambda_T) ):
                    j=j-1
                if (i<j):            
                    x[j]=x[i]
                    i=i+1;j=j-1
...
def S1S2_QASB(v,n,z):
    v = [v[i][0] for i in range(n)]
    delta = 1e-9
    U = np.array([0 for i in range(10)]); x = [0 for i in range(n)]
    flag=0
...
def prox_B1B2(x,t):
    ...
    vmax = max(abs(x))[0]
    fanshu2 = math.sqrt(np.dot(x.T,x)[0][0])
    fanshu1 = sum(abs(x))[0]
    if (fanshu2<=1) and (fanshu1<=t):
        for i in range(p):
            out[i][0] = x[i][0]
    elif (fanshu2>1) and (fanshu1<=t*fanshu2):
        for i in range(p):
            out[i][0] = x[i][0]/fanshu2
    elif (fanshu1>t) and (fanshu1>t*fanshu2):
        x_n = eplb(x,p,t,vmax/2)
        fanshu2_x_n = math.sqrt(np.dot(x_n.T,x_n)[0][0])
        if (fanshu2_x_n<=1):
            out = x_n
        else:
            x1 = S1S2_QASB(x, p, t)
